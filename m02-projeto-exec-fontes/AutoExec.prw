#INCLUDE 'PROTHEUS.CH'
#INCLUDE 'RWMAKE.CH'
#INCLUDE 'FONT.CH' // Include para fontes coloridas
#INCLUDE 'COLORS.CH' // Include para cores

User Function AutoExec()
    Private cNomeFun   := Space(30)


    SetPrvt("oFont1","oFont2","oFont3","oDlg1","oSay1","oSay2","oSay3","oSay4","oSay5","oSay6","oSay7","oSay8")
    ...
    oDlg1      := MSDialog():New( 092,232,592,927,"Auto Exec - Executando Programas ADVPL",,,.F.,,,,,,.T.,,,.T. )

    oSay1      := TSay():New( 032,008,{||"Digite o seu fonte sem o U_"},oDlg1,,oFont1,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,132,016)
    oSay2      := TSay():New( 100,008,{||"Caso tenha parâmetros, informe-os entre parênteses"},oDlg1,,oFont2,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,164,008)
    oSay3      := TSay():New( 113,009,{||"Exemplos:"},oDlg1,,oFont2,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,047,008)
    oSay4      := TSay():New( 154,010,{||"Soma(2,3)"},oDlg1,,oFont2,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,047,008)
    oSay5      := TSay():New( 167,011,{||'fNome("Exemplo")'},oDlg1,,oFont2,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,073,008)
    oSay6      := TSay():New( 127,011,{||"zTSay()"},oDlg1,,oFont2,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,047,008)
    oSay7      := TSay():New( 140,012,{||"Protjeto01()"},oDlg1,,oFont2,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,047,008)
    oSay8      := TSay():New( 180,012, {||"TRFORNECE()"},oDlg1,,oFont2,.F.,.F.,.F.,.T.,CLR_BLACK,CLR_WHITE,047,008)
